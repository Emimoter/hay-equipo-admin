...
async function seed() {
  ...
  // 1. Guardar clubes en settings/hay_equipo_clubs
  console.log(`\n📍 Guardando ${MDP_REAL_CLUBS.length} complejos reales de Mar del Plata en Firestore (settings/hay_equipo_clubs):`);
  await setDoc(doc(db, 'settings', 'hay_equipo_clubs'), {
    clubs: MDP_REAL_CLUBS,
    totalClubs: MDP_REAL_CLUBS.length,
    updatedAt: new Date().toISOString()
  });
  for (const c of MDP_REAL_CLUBS) {
    console.log(`  ✓ [CLUB] ${c.name} - ${c.address}, ${c.city} (${c.latitude}, ${c.longitude})`);
  }

  // 2. Guardar canchas en settings/hay_equipo_courts
  console.log(`\n🎾⚽ Guardando ${MDP_REAL_COURTS.length} canchas de Pádel y Fútbol en Firestore (settings/hay_equipo_courts):`);
  await setDoc(doc(db, 'settings', 'hay_equipo_courts'), {
    courts: MDP_REAL_COURTS,
    totalCourts: MDP_REAL_COURTS.length,
    updatedAt: new Date().toISOString()
  });
  for (const ct of MDP_REAL_COURTS) {
    console.log(`  ✓ [CANCHA] ${ct.name} - Deporte: ${ct.sportType} (Club: ${ct.clubId})`);
  }

  // 3. Dejar turnos vacíos (sin turnos disponibles actualmente, como fue solicitado)
  console.log(`\n⏰ Configurando slots vacíos en Firestore (settings/hay_equipo_slots)...`);
  ...
  // 4. Limpiar reservas mock del usuario
  try {
    await setDoc(doc(db, 'users', user.uid, 'hay_equipo_data', 'turnos_y_reservas'), {
      fixedSlotSubscription: null,
      activeBooking: null,
      updatedAt: new Date().toISOString()
    });
  } catch (e) {
    console.log('User booking clean note:', e.message);
  }

  console.log('\n🎉 ¡Base de datos Firestore actualizada con las canchas reales de Mar del Plata!');
}
