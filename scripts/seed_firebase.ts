...
// Generate courts for each club (each club has Padel courts + Futbol courts)
export const MDP_REAL_COURTS: any[] = [];
...
async function cleanAndSeedFirestore() {
  console.log('🧹 1. Eliminando datos mock up anteriores de Firestore (pclink-f6e0d)...');
  ...
  console.log(`📍 2. Guardando ${MDP_REAL_CLUBS.length} complejos reales de Mar del Plata en colección 'clubs'...`);
  for (const club of MDP_REAL_CLUBS) {
    await setDoc(doc(db, 'clubs', club.id), club);
    console.log(`  ✓ Club guardado: ${club.name} -> Lat: ${club.latitude}, Lng: ${club.longitude}`);
  }

  console.log(`🎾⚽ 3. Guardando ${MDP_REAL_COURTS.length} canchas (Pádel y Fútbol) en colección 'courts'...`);
  for (const court of MDP_REAL_COURTS) {
    await setDoc(doc(db, 'courts', court.id), court);
    console.log(`  ✓ Cancha guardada: ${court.name} (${court.sportType}) en ${court.clubId}`);
  }

  console.log('🔒 4. No se generaron turnos disponibles (colección slots vacía como fue solicitado).');
  console.log('🔒 5. Números de teléfono omitidos de las entidades públicas.');
  console.log('🎉 ¡Carga real de Mar del Plata completada exitosamente en Firebase!');
}
